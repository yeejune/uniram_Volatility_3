...
def detect_ssdt_hooks(report):
    ...
    for line in ssdt_output.strip().split('\n'):
        ...
        # 모듈명 추출
        module = parts[-1].lower().strip() if parts else ""
        ...
        if not is_known and module and not module.startswith('-'):
            findings.append(Finding(
                category="SSDT 후킹",
                severity="CRITICAL",
                title=f"SSDT 후킹 탐지: {module}",
                detail=f"비정상 모듈이 시스템 콜을 후킹하고 있습니다.",
                evidence=f"원본: {line.strip()}"
            ))
...
def detect_suspicious_services(report):
    ...
    if not svcscan_output:
        print("  [!] svcscan 결과가 없습니다.")
        return []

    SUSPICIOUS_SVC_PATTERNS = [
        (r'\\temp\\', 'Temp 경로에서 실행되는 서비스', 70),
        (r'\\tmp\\', 'Tmp 경로에서 실행되는 서비스', 70),
        (r'\\appdata\\', 'AppData 경로에서 실행되는 서비스', 60),
        (r'\\users\\public\\', 'Public 폴더에서 실행되는 서비스', 60),
        (r'\\downloads\\', 'Downloads 폴더에서 실행되는 서비스', 50),
        (r'\\desktop\\', 'Desktop에서 실행되는 서비스', 50),
        (r'powershell', 'PowerShell 기반 서비스', 60),
        (r'cmd\.exe', 'CMD 기반 서비스', 50),
        (r'mshta', 'MSHTA 기반 서비스', 70),
        (r'wscript', 'WScript 기반 서비스', 60),
        (r'cscript', 'CScript 기반 서비스', 60),
        (r'rundll32', 'Rundll32 기반 서비스', 40),
        (r'regsvr32', 'Regsvr32 기반 서비스', 50),
        (r'[a-f0-9]{8,}\.exe', '랜덤 해시명 실행 파일', 60),
        (r'[a-z]{1}\.exe', '한 글자 실행 파일명', 70),
    ]
    ...
    for line in svcscan_output.strip().split('\n'):
        ...
        for pattern, desc, pts in SUSPICIOUS_SVC_PATTERNS:
            if re.search(pattern, line_lower, re.IGNORECASE):
                score += pts
                reasons.append(desc)

        if score >= 50:
            severity = "CRITICAL" if score >= 100 else "HIGH" if score >= 70 else "MEDIUM"
            ...
            findings.append(Finding(
                category="서비스",
                severity=severity,
                title=f"의심 서비스 탐지: {svc_name}",
                detail=f"사유: {', '.join(reasons)}",
                evidence=f"원본: {line.strip()}"
            ))
...
def detect_suspicious_registry(report):
    """의심 레지스트리 탐지"""
    # printkey 또는 hivelist 결과 활용
    printkey_output = report.raw_outputs.get("printkey", "")
    hivelist_output = report.raw_outputs.get("hivelist", "")

    combined_output = printkey_output + "\n" + hivelist_output
    if not combined_output.strip():
        print("  [!] 레지스트리 결과가 없습니다.")
        return []

    PERSISTENCE_KEYS = [
        (r'CurrentVersion\\Run\b', '자동 실행 등록', 'HIGH'),
        (r'CurrentVersion\\RunOnce', '일회성 자동 실행', 'HIGH'),
        (r'CurrentVersion\\RunServices', '서비스 자동 실행', 'HIGH'),
        (r'Winlogon\\Shell', '로그인 쉘 변경', 'CRITICAL'),
        (r'Winlogon\\Userinit', 'Userinit 변경', 'CRITICAL'),
        (r'Winlogon\\Notify', 'Winlogon 알림 변경', 'CRITICAL'),
        (r'CurrentVersion\\Policies\\Explorer\\Run', '정책 자동 실행', 'HIGH'),
        (r'CurrentVersion\\Explorer\\Shell Folders', '쉘 폴더 변경', 'MEDIUM'),
        (r'CurrentVersion\\Explorer\\User Shell Folders', '사용자 쉘 폴더 변경', 'MEDIUM'),
        (r'Environment\\UserInitMprLogonScript', '로그온 스크립트', 'CRITICAL'),
        (r'Control\\Session Manager\\BootExecute', '부트 실행 변경', 'CRITICAL'),
        (r'Control\\Session Manager\\AppCertDlls', 'AppCertDlls 등록', 'CRITICAL'),
        (r'Control\\Session Manager\\KnownDLLs', 'KnownDLLs 변경', 'HIGH'),
        (r'Classes\\CLSID.*\\InprocServer32', 'COM 하이재킹', 'HIGH'),
        (r'Image File Execution Options', 'IFEO 디버거 등록', 'CRITICAL'),
        (r'SilentProcessExit', '프로세스 종료 모니터', 'HIGH'),
        (r'AppInit_DLLs', 'AppInit DLL 등록', 'CRITICAL'),
        (r'BootVerificationProgram', '부트 검증 프로그램', 'HIGH'),
        (r'ServiceDll', '서비스 DLL 등록', 'MEDIUM'),
        (r'Security Providers', '보안 제공자 변경', 'HIGH'),
        (r'Installed Components.*StubPath', '설치 컴포넌트 StubPath', 'HIGH'),
    ]

    SUSPICIOUS_VALUES = [
        (r'powershell', 'PowerShell 참조', 'HIGH'),
        (r'cmd\.exe.*/c', 'CMD 명령 실행', 'HIGH'),
        (r'mshta', 'MSHTA 참조', 'HIGH'),
        (r'wscript', 'WScript 참조', 'MEDIUM'),
        (r'cscript', 'CScript 참조', 'MEDIUM'),
        (r'rundll32', 'Rundll32 참조', 'MEDIUM'),
        (r'regsvr32', 'Regsvr32 참조', 'MEDIUM'),
        (r'\\temp\\', 'Temp 경로 참조', 'HIGH'),
        (r'\\tmp\\', 'Tmp 경로 참조', 'HIGH'),
        (r'\\appdata\\', 'AppData 경로 참조', 'MEDIUM'),
        (r'\\users\\public\\', 'Public 폴더 참조', 'HIGH'),
        (r'\\downloads\\', 'Downloads 폴더 참조', 'MEDIUM'),
        (r'\\desktop\\', 'Desktop 경로 참조', 'MEDIUM'),
        (r'base64', 'Base64 문자열', 'HIGH'),
        (r'invoke-', 'PowerShell Invoke', 'HIGH'),
        (r'iex\s*\(', 'PowerShell IEX', 'HIGH'),
        (r'-enc\s', 'PowerShell 인코딩', 'CRITICAL'),
        (r'http://', 'HTTP URL 참조', 'MEDIUM'),
        (r'https://', 'HTTPS URL 참조', 'LOW'),
        (r'ftp://', 'FTP URL 참조', 'HIGH'),
        (r'[a-f0-9]{8,}\.exe', '랜덤 해시명 실행파일', 'HIGH'),
        (r'[a-z]{1}\.exe', '한 글자 실행파일', 'HIGH'),
    ]
    ...
    for line in combined_output.strip().split('\n'):
        ...
        # 1) 지속성 관련 레지스트리 키 탐지
        for pattern, desc, severity in PERSISTENCE_KEYS:
            if re.search(pattern, line_stripped, re.IGNORECASE):
                findings.append(Finding(
                    category="레지스트리",
                    severity=severity,
                    title=f"지속성 레지스트리 키: {desc}",
                    detail=f"키 패턴: {pattern}",
                    evidence=f"원본: {line_stripped[:300]}"
                ))
                break

        # 2) 의심 값 탐지
        for pattern, desc, severity in SUSPICIOUS_VALUES:
            if re.search(pattern, line_stripped, re.IGNORECASE):
                # 지속성 키에서 이미 탐지된 행은 중복 방지
                already_found = any(
                    f.evidence and line_stripped[:100] in f.evidence
                    for f in findings
                )
                if not already_found:
                    findings.append(Finding(
                        category="레지스트리",
                        severity=severity,
                        title=f"의심 레지스트리 값: {desc}",
                        detail=f"값 패턴: {pattern}",
                        evidence=f"원본: {line_stripped[:300]}"
                    ))
                break
